...
async fn failed_final_commit_preserves_pending_state_and_returns_failure() {
    let _guard = ENV_LOCK.lock().unwrap();
    let root = unique_tmp("nrr-test/nrr-final-commit-close");
        ...
        while let Some(message) = socket.next().await {
            ...
            if text.contains("\"input_audio_buffer.commit\"") {
                ...
                socket
                    ...
                    .unwrap();
                completed_sent = true;
            } else if completed_sent && text.contains("\"input_audio_buffer.append\"") {
    ...
    assert!(error.contains("final audio commit send failed"));
    assert_eq!(commits_received, 1);
    assert_eq!(
        drain["captureStats"]["realtimeCompletedSegmentsObserved"],
        1
    );
    assert_eq!(drain["captureStats"]["audioChunksSent"], 3);
    ...
    assert_eq!(drain["finalCommit"]["status"], "send-failed");
    assert!(
        drain["errors"][0]
            .as_str()
            .unwrap()
            .contains("final audio commit")
    );
